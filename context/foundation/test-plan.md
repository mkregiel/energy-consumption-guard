...
| 4   | Auth boundary + CI gate           | Prove unauthenticated requests to config endpoints are rejected; wire all tests into GitHub Actions CI on PR                      | R6               | contract/integration (negative auth), CI config                  | complete      | context/changes/auth-boundary-ci-gate              |
| 5   | E2E critical user journeys        | Prove Tuya OAuth connect flow, auth redirect behaviour, and limit/email config round-trip work in a real browser                  | R-E1, R-E2, R-E3 | e2e (Playwright, DOM snapshot)                                   | change opened | context/changes/testing-e2e-critical-user-journeys |
