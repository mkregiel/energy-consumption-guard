...
- [x] 1.1 Dependency installs cleanly: `npm install` — e86e36b
- [x] 1.2 Type checking passes: `npm run typecheck` — e86e36b
- [x] 1.3 Linting passes: `npm run lint` — e86e36b
- [x] 1.4 Migrated breach-job suite passes under the Workers pool: `npm run test:ci -- breach-notifications-idempotency` — e86e36b
- [x] 1.5 Both pre-existing tests pass with original assertions intact — e86e36b

#### Manual

- [x] 1.6 Full suite run confirms pure-unit suites still run under `node` and pass — e86e36b
- [x] 1.7 Confirm breach-job suite actually executes inside `workerd` (not silently falling back to `node`) — e86e36b

### Phase 2: Retry / terminal-failure path coverage

#### Automated

- [x] 2.1 New test passes: `npm run test:ci -- breach-notifications` — 4d1d9ff
- [x] 2.2 Type checking passes: `npm run typecheck` — 4d1d9ff
- [x] 2.3 Linting passes: `npm run lint` — 4d1d9ff
- [x] 2.4 Existing two idempotency tests remain green — 4d1d9ff

#### Manual

- [x] 2.5 Review test assertions against `markBreachNotified`/attempt-recording logic for precision — 4d1d9ff

### Phase 3: Resend HTTP-error path coverage

#### Automated

- [x] 3.1 New test passes: `npm run test:ci -- breach-notifications` — 82defcc
- [x] 3.2 Type checking passes: `npm run typecheck` — 82defcc
- [x] 3.3 Linting passes: `npm run lint` — 82defcc
- [x] 3.4 All prior tests in the suite remain green — 82defcc

#### Manual

- [x] 3.5 Confirm mocked error shape mirrors real `email-client.ts:24-28` output — 82defcc
- [x] 3.6 Full suite (`npm run test:ci`) runs cleanly end to end (node + Workers pool) — 82defcc
